...
public class EmailVerification extends Model {
	...
	public EmailVerification(long userId, boolean isVerified){
		this.id = HashHelper.createPassword(userId + new Date().toString()).replace('/', '1');
		this.userId = userId;
		this.createdOn = new Date();
		this.isVerified = isVerified;
	}
	
	static Finder<String, EmailVerification> find = new Finder<String, EmailVerification>(String.class,
	...
	public static boolean isEmailVerified(long userId){
		 List<EmailVerification> list = find.where().eq("userId", userId).orderBy("createdOn").findList();
