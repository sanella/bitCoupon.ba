...
public class Global extends GlobalSettings {
	...
	public void onStart(Application app) {
		...
		if (!Category.checkByName("Food")) {
			food = new Category("Food");
			food.save();
		}
		if (!Category.checkByName("Travel")) {
			travel = new Category("Travel");
			travel.save();
		}

		if (!Category.checkByName("Sport")) {
		...
		if (Coupon.checkByName(nameCoupon1) == false) {
			Coupon.createCoupon(nameCoupon1, 80, null,
					...
					travel, descriptionCoupon1, remarkCoupon1);
		}
		if (Coupon.checkByName(nameCoupon2) == false) {
			Coupon.createCoupon(
					...
					sport, descriptionCoupon2, remarkCoupon2);
		}
		if (Coupon.checkByName(nameCoupon3) == false) {
			Coupon.createCoupon(
					...
					food, descriptionCoupon3, remarkCoupon3);
		}
